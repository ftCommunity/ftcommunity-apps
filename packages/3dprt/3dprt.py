...
PRINTER_VIDPID = "16c0:0483" # teensy
...
class PrinterThread(QThread):
    ...
    def run(self):
        ...
        if port == None:
            ...
            printer_ports = [ ]
            for p in serial.tools.list_ports.grep("vid:pid="+PRINTER_VIDPID):
                printer_ports.append(p.device)

            # check if we found a printer
            if len(printer_ports) == 0:
                self.s = None
                self.connection_failed.emit()
                return

            port = printer_ports[0]

        # found something that seems to be a printer. Try to connect ...
        try:
...
class ManualWidget(QWidget):
    ...
    def on_dir_clicked(self, str):
        self.cmd.emit("G91")    # set relative positioning mode
        self.cmd.emit("G0 " + str + " F3000")
        self.cmd.emit("G90")    # set absolute positioning mode
        self.cmd.emit("M114")   # get current position
