#! /usr/bin/env python3
import telepot, sys, pprint, time, ftrobopy, os, configparser
approot = (os.path.realpath(__file__).rpartition('/')[0] + os.path.realpath(__file__).rpartition('/')[1])
...
def handle(msg):
	...
	###FUNCTION WITH FREE VALUE
		...
		if is_number(command) == True:
			...
			bot.sendMessage(chat_id, 'This is not a value! Leaving Sound Menu!')
		function = ''
	else:
		bot.sendMessage(chat_id, 'Wrong Command! See /help for further information')
		print('Wrong Command')
