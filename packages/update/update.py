#! /usr/bin/env python3
import sys, os, os.path, time, urllib.request, ssl, json
...
raw_data = urllib.request.urlopen('https://api.github.com/repos/ftCommunity/ftcommunity-TXT/releases',context=ctx).read().decode()
...
if release_version != curreentver:
	...
	os.system("sudo wget -O " + sdcardbase + downloadzip + " --no-check-certificate " + dl_link)
	...
	os.system("sudo mv " + sdcardbase + "am335x-kno_txt.dtb" + " " + sdcardbase + olddir + "/am335x-kno_txt.dtb")
	os.system("sudo mv " + sdcardbase + "rootfs.img" + " " + sdcardbase + olddir + "/rootfs.img")
	os.system("sudo mv " + sdcardbase + "uImage" + " " + sdcardbase + olddir + "/uImage")
	#unzip
	os.system('(/bin/echo "msg Update! Do not plug off the power corde! Unzipping!" & /bin/echo "quit") | sudo /usr/bin/nc localhost 9000')
	os.system("sudo unzip -o " + sdcardbase + downloadzip + " -d " + sdcardbase)
